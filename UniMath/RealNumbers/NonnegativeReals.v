...
Proof.
  ...
  revert Hx ; apply hinhuniv ; apply sumofmaps ; intros Hx.
  - apply hinhpr.
    exists 1 ; exact Hx.
  - apply hinhpr ; exists (pr1 Hx + 1) ; exact (pr2 (pr2 Hx)).
Qed.
...
Proof.
  ...
  generalize (is_Dcuts_open z (pr1 r) (pr2 (pr2 r))) ; apply hinhuniv ; intros r'.
  assert (Hr0 : 0%NRat < pr1 r' - pr1 r).
  { apply ispositive_minusNonnegativeRationals.
    exact (pr2 (pr2 r')). }
  generalize (is_Dcuts_error y _ Hr0) ; apply hinhuniv ; apply sumofmaps ; intros Yq.
  - apply Utilities.squash_element ;
    ...
    + exact (pr1 (pr2 r')).
  - generalize (isdecrel_leNonnegativeRationals (pr1 Yq + (pr1 r' - pr1 r)) (pr1 r')).
    apply sumofmaps ; intros Hdec.
    + apply hinhpr ; right ; apply hinhpr.
      exists (pr1 r') ; split.
      intro Yr' ; apply (pr2 (pr2 Yq)).
      apply is_Dcuts_bot with (pr1 r').
      exact Yr'.
      exact Hdec.
      exact (pr1 (pr2 r')).
    + apply hinhpr ; left ; apply hinhpr.
      ...
      * intro Xq ; apply (pr1 (pr2 r)).
        apply is_Dcuts_bot with (pr1 Yq).
        exact Xq.
...
Proof.
  ...
  - intros Hnlt y Yy.
    ...
    generalize (pr1 (ispositive_minusNonnegativeRationals _ _) (pr2 (pr2 y'))) ; intro Hy.
    generalize (is_Dcuts_error X _ Hy) ; apply hinhuniv ; apply sumofmaps ; [intros nXc | ].
...
Proof.
  intros x.
  unfold neg ; apply sumofmaps ; intros Hap.
...
Proof.
  intros q r Hr.
  apply hinhpr.
  set (n := between_ltNonnegativeRationals r q Hr).
  exists (pr1 n).
  split.
  exact (pr2 (pr2 n)).
  exact (pr1 (pr2 n)).
...
Proof.
  intros r Hr n Hn.
  revert Hr ; apply hinhfun ; apply sumofmaps ; [ apply sumofmaps ; intros Hr | intros xy].
  - left ; left.
  ...
  - right.
    ...
    + set (nx := ((pr1 (pr1 xy)) * (n / r))%NRat).
      ...
      exists (nx,,ny).
      repeat split.
      * change (n = pr1 (pr1 xy) * (n / r) + pr2 (pr1 xy) * (n / r)).
...
Proof.
  intros r Hr n Hn.
  revert Hr ; apply hinhfun ; intros ry.
  generalize (isdeceq_NonnegativeRationals r 0%NRat) ; apply sumofmaps ; intros Hr0.
  - rewrite Hr0 in Hn.
    apply NonnegativeRationals_eq0_le0 in Hn.
    exists 0%NRat.
...
Proof.
  ...
  intros x y.
  exists (pr1 x * pr1 y).
  unfold neg ; apply (hinhuniv (P := hProppair _ isapropempty)) ; intros xy.
...
Proof.
  ...
  - apply hinhpr ; left.
    ...
    apply multNonnegativeRationals_ltcompat.
    apply notge_ltNonnegativeRationals ; intro H.
    now apply Hx1, X_bot with (1 := pr1 (pr2 (pr2 xy))).
    apply notge_ltNonnegativeRationals ; intro H.
  ...
  - rename Hy into y.
    ...
    + left.
      ...
      now apply (pr2 (pr2 y)), Y_bot with (1 := pr2 (pr2 (pr2 xy))).
      apply notge_ltNonnegativeRationals ; intro H0.
    ...
    + right.
      ...
      * unfold neg ; apply (hinhuniv (P := hProppair _ isapropempty)) ;
        ...
        apply multNonnegativeRationals_ltcompat.
        apply notge_ltNonnegativeRationals.
        now intros H ; apply (pr2 (pr2 x)), X_bot with (1 := pr1 (pr2 (pr2 xy))).
        apply notge_ltNonnegativeRationals.
        now intros H ; apply (pr2 (pr2 y)), Y_bot with (1 := pr2 (pr2 (pr2 xy))).
...
Proof.
  ...
  - assert (Hx1 : (0 < pr1 x + 1)%NRat).
    ...
    { apply funextfun ; intro r.
      ...
      - apply hinhuniv ; intros rx.
        generalize (pr2 (pr2 rx)).
        apply hinhuniv ; intros xy.
        generalize (pr1 (pr2 (pr2 xy))).
        apply hinhfun ; intros rx'.
        rewrite (pr1 (pr2 rx)), (pr1 (pr2 xy)), (pr1 (pr2 rx')).
        exists (pr1 rx',snd (pr1 xy)) ; repeat split ; simpl.
...
Proof.
  ...
  generalize ((pr1 (pr2 l)) _ Xx) ; intro H.
...
Proof.
  ...
  - rewrite Hr0 in l |- * ; clear r Hr0.
    ...
    + apply ispositive_invNonnegativeRationals.
      now apply ispositive_multNonnegativeRationals.
  - generalize (between_ltNonnegativeRationals _ _ (pr2 (pr2 (pr2 l)))) ; intros l'.
    apply hinhpr.
...
Proof.
  generalize (X_open _ X_0) ; apply (hinhuniv (P := hProppair _ (isaprop_Dcuts_def_error _))) ; intros x ; simpl.
  ...
  { apply funextfun ; intro r.
    ...
    - apply hinhfun ; intros l.
      ...
      exists (pr1 l) ; repeat split.
      intros q ; unfold Y.
      apply hinhuniv ; intros s ; rewrite (pr1 (pr2 s)).
      ...
      exact (pr1 (pr2 (pr2 l))).
      exact (pr2 (pr2 (pr2 l))).
    - apply hinhuniv ; intros q ; rewrite (pr1 (pr2 q)) ; generalize (pr2 (pr2 q)).
      apply hinhfun ; intros l.
      exists (pr1 l) ; repeat split.
      intros s Xs.
  ...
  rewrite Heq.
  apply Dcuts_NQmult_error.
  now apply ispositive_invNonnegativeRationals, (pr2 (pr2 x)).
...
Proof.
  ...
  exists (fst (pr1 xy)) ; split.
  intro H ; apply (pr1 (pr2 r)).
  apply hinhpr ; exists (pr1 xy) ; repeat split.
  exact (pr1 (pr2 xy)).
  exact H.
  exact (pr2 (pr2 (pr2 xy))).
  exact (pr1 (pr2 (pr2 xy))).
  ...
  - apply hinhuniv, sumofmaps ; simpl pr1.
    + apply sumofmaps.
      ...
      * intros Hr.
        ...
        now apply hinhpr ; left ; right.
    + intros xy.
      generalize (pr1 (pr2 (pr2 xy))) ; apply hinhuniv, sumofmaps.
      * apply sumofmaps ; intros Hxy.
        ...
        { apply hinhpr ; left ; right.
          ...
          - exact (pr2 (pr2 (pr2 xy))). }
      * intros xy'.
        apply hinhpr ; right ; exists (pr1 (pr1 xy'),,pr2 (pr1 xy') + pr2 (pr1 xy)).
        repeat split ; simpl.
        { pattern r at 1 ; rewrite (pr1 (pr2 xy)).
          rewrite (pr1 (pr2 xy')).
          now apply isassoc_plusNonnegativeRationals. }
        { exact (pr1 (pr2 (pr2 xy'))). }
        { apply hinhpr ; right ; exists (pr2 (pr1 xy'),,(pr2 (pr1 xy))).
          repeat split.
          - exact (pr2 (pr2 (pr2 xy'))).
          - exact (pr2 (pr2 (pr2 xy))). }
  - apply hinhuniv, sumofmaps.
    ...
    + intros xz.
      generalize (pr2 (pr2 (pr2 xz))) ; apply hinhuniv, sumofmaps.
      * apply sumofmaps ; intros Hyz.
        { apply hinhpr ; left ; left.
          apply hinhpr ; right ; exists (pr1 xz).
          repeat split.
          - exact (pr1 (pr2 xz)).
          - exact (pr1 (pr2 (pr2 xz))).
          - exact Hyz. }
        { apply hinhpr ; right ; exists (pr1 xz).
          repeat split.
          - exact (pr1 (pr2 xz)).
          ...
          - exact Hyz. }
      * intros yz.
        apply hinhpr ; right ; exists ((pr1 (pr1 xz) +pr1 (pr1 yz)),, pr2 (pr1 yz)).
        repeat split ; simpl.
        { pattern r at 1 ; rewrite (pr1 (pr2 xz)), (pr1 (pr2 yz)).
          now rewrite isassoc_plusNonnegativeRationals. }
        { apply hinhpr ; right ; exists (pr1 (pr1 xz),,pr1 (pr1 yz)).
          repeat split.
          - exact (pr1 (pr2 (pr2 xz))).
          - exact (pr1 (pr2 (pr2 yz))). }
        { exact (pr2 (pr2 (pr2 yz))). }
...
Proof.
  ...
  - apply hinhuniv ; intros xyz.
    ...
    + exact (pr1 (pr2 (pr2 xy))).
    + apply hinhpr ; exists (snd (pr1 xy),snd (pr1 xyz)) ; repeat split.
      exact (pr2 (pr2 (pr2 xy))).
  ...
  - apply hinhuniv ; intros xyz.
    ...
    rewrite (pr1 (pr2 yz)), <- isassoc_multNonnegativeRationals.
    exists ((fst (pr1 xyz) * fst (pr1 yz)) , snd (pr1 yz)) ; simpl ; repeat split.
    + apply hinhpr ; exists (fst (pr1 xyz),fst (pr1 yz)) ; repeat split.
...
Proof.
  ...
  - apply hinhuniv ; intros ix.
    ...
    pattern r at 1 ;
      rewrite (pr1 (pr2 ix)), iscomm_multNonnegativeRationals.
...
Proof.
  ...
  - apply hinhuniv ; intros (r,(nYr,Zr)).
    ...
    + apply hinhpr ; exists r' ; split.
      * unfold neg ; apply (hinhuniv (P := hProppair _ isapropempty)) ; intros [ | ] ; [intros [Yr' | Xr'] | intros ((ry,rx),(Hr',(Yr',Xr')))].
...
Proof.
  ...
  apply Dcuts_eq_is_eq.
  intros r ; split.
  - apply hinhfun ; intros [ | ] ; [intros [Xr | Xr] | intros ((rx,ry)) ; simpl ; intros (->,(Xr,Yr))].
    + exists (r / 2)%NRat.
      simpl ; split.
...
Proof.
  ...
  - apply hinhpr ; exists x' ; split.
    ...
    + intros y Yy.
      ...
      * rewrite isassoc_plusNonnegativeRationals, (iscomm_plusNonnegativeRationals _ y), <- isassoc_plusNonnegativeRationals.
        apply istrans_lt_le_ltNonnegativeRationals with (x + (x' - x)).
  ...
  - destruct Hx as (x,(Xx,nXx)).
    ...
    + assert (HY : Π y', coprod (Y y') (y' = 0%NRat) -> (y' < y + c / 2)%NRat).
      ...
      * unfold neg ; apply (hinhuniv (P := hProppair _ isapropempty)) ; intros (x',(Xx',Hx')).
        ...
        rewrite isassoc_plusNonnegativeRationals, (iscomm_plusNonnegativeRationals _ y).
        pattern c at 3 ; rewrite (NQhalf_double c).
        rewrite <- (isassoc_plusNonnegativeRationals y), <- isassoc_plusNonnegativeRationals.
        rewrite minusNonnegativeRationals_plus_r.
    ...
    + apply notge_ltNonnegativeRationals in Hxy.
      ...
      apply_pr2 notlt_geNonnegativeRationals.
      case (isdecrel_leNonnegativeRationals y x') ; intro Hxy'.
      rewrite iscomm_plusNonnegativeRationals, (NQhalf_double c), <- isassoc_plusNonnegativeRationals.
      apply istrans_leNonnegativeRationals with (x + c / 2)%NRat.
...
Proof.
  ...
  - intros Zr.
    ...
    + intros (y,(Yy,Hy)).
      apply hinhpr.
      exists (y + q) ; split.
      apply hinhpr ; right ; exists (y,,q) ; simpl ; repeat split.
      * exact Yy.
      * exact Zq.
      * intros y' [Yy' | Hy0].
        apply minusNonnegativeRationals_ltcompat_l' with r.
  ...
  - apply hinhuniv ; intros (q,(YZq,Hq)).
    revert YZq ; apply hinhuniv ; intros [ | ] ; [intros [ Yq | Zq ] | intros ((qy,qz)) ; simpl ; intros (H,(Yq,Zq)) ].
    + apply fromempty, (isnonnegative_NonnegativeRationals' r).
      apply_pr2 (plusNonnegativeRationals_ltcompat_r q).
      rewrite islunit_zeroNonnegativeRationals.
      apply Hq.
      now left.
    + apply is_Dcuts_bot with (1 := Zq), lt_leNonnegativeRationals.
      ...
      now right.
    + apply is_Dcuts_bot with (1 := Zq), lt_leNonnegativeRationals.
      apply_pr2 (plusNonnegativeRationals_ltcompat_l qy).
      rewrite <- H, iscomm_plusNonnegativeRationals.
      apply Hq.
      now left.
Qed.
...
Proof.
  ...
  apply Dcuts_eq_is_eq.
  intros r ; split.
  - apply hinhuniv ; intros [ | ] ; [ intros [Zr | ]
    | intros ((rz,rxy)) ; simpl fst ; simpl snd ; intros (->,(Zr)) ].
    + apply hinhpr.
      left.
      apply hinhpr.
  ...
  - apply hinhuniv ; intros [ | ] ; apply hinhuniv ; intros [ | ].
    + intros [Zr | Xr] ; apply hinhpr ; left.
      * now left.
      * right.
        apply hinhpr.
        now left.
    + intros ((rz,rx)) ; intros (->,(Zr,Xr)) ; apply hinhpr.
      right.
      exists (rz,, rx).
      repeat split.
      exact Zr.
      apply hinhpr.
      now left.
    + intros [Zr | Yr] ; apply hinhpr ; left.
      * now left.
      * right.
        apply hinhpr.
        now right.
    + intros ((rz,ry)) ; intros (->,(Zr,Yr)) ; apply hinhpr.
      right.
      exists (rz,, ry).
      repeat split.
      exact Zr.
...
Proof.
  ...
  apply Dcuts_eq_is_eq.
  intros r ; split.
  - apply hinhfun ; intros [ | ] ; [intros [Xr | Yr] | intros ((rx,ry)) ; simpl ; intros (Hr,(Xr,Yr))].
    + left.
      left.
      exact Xr.
    + left.
    ...
    + right.
      ...
      * rewrite <- NQhalf_double.
        exact Yr.
  - apply hinhfun ; intros [ | ] ; [intros [Xr | Yr] | intros ((rx,ry)) ; simpl ; intros (->,(Xr,Yr))].
    + left.
      left.
      exact Xr.
    + left.
...
Proof.
  ...
  - intros Hr.
    ...
    now repeat split.
  - apply hinhuniv ; intros [|] ; [intros [ | ] | intros ((rx,ry)) ; simpl ; intros (Hr,(Xr,Yr)) ].
...
Proof.
  ...
  - apply hinhpr ; left.
    ...
    + specialize (HuN' _ (isreflnatleh _)).
      ...
      apply hinhuniv ; intros [ | ] ; [ intros [H | H] | intros ((rx,ry)) ; simpl ; intros (Hr,(Xr,Yr))].
  ...
  - destruct HuN as (q,(UNq,nUNq)).
    ...
    +  apply hinhpr ; right.
      ...
      * apply hinhpr.
        ...
        apply hinhuniv ; intros [ | ] ; [intros [Xr | Yr] | intros ((rx,ry)) ; simpl ; intros (Hr,(Xr,Yr))].
        { apply (U_bot n) with (1 := Xr).
          pattern q at 2 ;
  ...
  - eapply istrans_Dcuts_lt_le_rel.
    ...
    + intros r Hr.
      ...
      * apply notlt_geNonnegativeRationals in Hrc.
        ...
        apply hinhpr ; right ; exists (r - c / 2%NRat,, c / 2%NRat) ; repeat split.
        ...
        apply hinhuniv ; intros [|] ; [ intros [Xr | Yr] | intros ((rx,ry)) ; simpl ; intros (->,(Xr,Yr))].
        apply is_Dcuts_bot with (1 := Xr).
        now apply minusNonnegativeRationals_le.
        simpl in Yr.
  ...
  - apply istrans_Dcuts_le_lt_rel with (Dcuts_plus (U N) (Dcuts_half eps)).
    + intros r.
      ...
      * apply hinhpr ; left ; right.
        ...
        now apply lt_leNonnegativeRationals, plusNonnegativeRationals_ltcompat ; apply Hrc.
      * apply notlt_geNonnegativeRationals in Hrc.
        apply hinhpr ; right ; exists (r - c / 2%NRat,, c / 2%NRat) ; simpl ; repeat split.
        now rewrite minusNonnegativeRationals_plus_r.
...
Proof.
  ...
  - apply hinhuniv ; intros (X,(EX,Hx)).
    ...
    + left.
      ...
      2: exact Yc.
      intros H ; apply Xc.
      revert H ; apply hinhuniv ; intros [ | ] ; [intros [Xc' | Yc'] | ].
  ...
  - intros Hr.
    ...
    + assert (Hxy : (0 < x + y)%NRat).
      ...
      * unfold divNonnegativeRationals.
        ...
        exact Hr.
        exact Hxy.
  - apply hinhuniv ; intros [ | ] ; [intros [Hrx | Hry] | intros ((rx,ry)) ; simpl ; intros (->,(Hrx,Hry))] ; simpl.
