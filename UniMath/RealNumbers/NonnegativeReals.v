...
Proof.
  ...
  apply hinhfun2.
  intros r q.
  exists (minNonnegativeRationals (pr1 r) (pr1 q)) ; split.
  + apply minNonnegativeRationals_case.
    exact (pr1 (pr2 r)).
    exact (pr1 (pr2 q)).
  + split.
    * apply is_Dcuts_bot with (1 := pr2 (pr2 r)), minNonnegativeRationals_ge_l.
...
Proof.
  ...
  exists (maxNonnegativeRationals (pr1 rx) (pr1 ry)) ; split.
...
Proof.
  ...
  apply hinhfun2.
  intros nx ny.
  exists (pr1 nx + pr1 ny) ; split.
  - apply hinhpr ; apply (mkDcuts_plus_acc _ _ _ (pr1 nx) (pr1 ny)).
    * reflexivity.
    * exact (pr1 (pr2 nx)).
    * exact (pr1 (pr2 ny)).
  - pattern r at 1 ;
