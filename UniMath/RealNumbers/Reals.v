...
Proof.
  change (hr_ap_rel 1%rng 0%rng).
  rewrite <- (hr_to_NR_bij 1%rng), <- (hr_to_NR_bij 0%rng), hr_to_NR_one, hr_to_NR_zero.
  apply NR_to_hr_ap.
  rewrite !isrunit_zero_plusNonnegativeReals.
...
Proof.
  ...
  unfold Rminus, CFminus, CFplus, CFopp, CCDRplus, CDRplus, Apartness.op1 ; simpl.
  apply maponpaths, (map_on_two_paths op1), maponpaths.
  reflexivity.
  reflexivity.
...
Proof.
  ...
  - intros (x,Hx) (y,Hy).
    exists (NRNRtoR x y).
    apply is_filter_lim_NRNRtoR.
    exact Hx.
    exact Hy.
  - apply (UniformSpace.isCauchy_filter_im (X := _ ,, US_Reals) (Y := _ ,, US_NonnegativeReals)), Hf.
    intros U.
    apply hinhfun.
    intros (e,(He,Hv)).
    exists e ; split.
