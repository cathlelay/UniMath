...
Definition is_pointed_subdivision (a b : M) (n : nat) (lx ly : nat -> M) :=
  lx O = a × (∀ k : nat, n ≤ k -> lx k = b)
  × (∀ k l m : nat, k ≤ l -> l ≤ m -> (dist (lx k) (lx m) = dist (lx k) (lx l) + dist (lx l) (lx m))%addmonoid)
  × (∀ k : nat, (dist (lx k) (ly k) + dist (ly k) (lx (S k)) = dist (lx k) (lx (S k)))%addmonoid).

Definition pointed_subdivision (a b : M) :=
  Σ (n : nat) (lx ly : nat -> M), is_pointed_subdivision a b n lx ly.

Definition ps_size {a b : M} (s : pointed_subdivision a b) : nat := pr1 s.
Definition ps_lx {a b : M} (s : pointed_subdivision a b) : (nat -> M) := pr1 (pr2 s).
Definition ps_ly {a b : M} (s : pointed_subdivision a b) : (nat -> M) := pr1 (pr2 (pr2 s)).

Lemma ps_lx_O {a b : M} (s : pointed_subdivision a b) : ps_lx s O = a.
Proof.
  intros a b s.
  apply (pr1 (pr2 (pr2 (pr2 s)))).
Qed.
Lemma ps_lx_last {a b : M} (s : pointed_subdivision a b) : ps_lx s (ps_size s) = b.
Proof.
  intros a b s.
  apply (pr1 (pr2 (pr2 (pr2 (pr2 s))))).
  now apply isreflnatleh.
Qed.
Lemma ps_lx_last' {a b : M} (s : pointed_subdivision a b) : ∀ k : nat, ps_size s ≤ k -> ps_lx s k = b.
Proof.
  intros a b s.
  apply (pr1 (pr2 (pr2 (pr2 (pr2 s))))).
Qed.

Lemma ps_lx_Chasles {a b : M} (s : pointed_subdivision a b) :
  ∀ k l m : nat, k ≤ l -> l ≤ m -> (dist (ps_lx s k) (ps_lx s m) = dist (ps_lx s k) (ps_lx s l) + dist (ps_lx s l) (ps_lx s m))%addmonoid.
Proof.
  intros a b s.
  apply (pr1 (pr2 (pr2 (pr2 (pr2 (pr2 s)))))).
Qed.

Lemma ps_lx_point {a : M} (s : pointed_subdivision a a) :
  ∀ k : nat, ps_lx s k = a.
Proof.
  intros a (n,(lx,(ly,(Ha,(Hb,(Hlx,Hly)))))).
  unfold ps_size, ps_lx.
  simpl pr1.
  clear ly Hly.
  intros k.
  destruct (natgthorleh n k) as [Hk | Hk].
  2: now apply Hb.
  apply istighttightapSet ; intros H.
  apply issepp_dist in H.
  apply (isirrefltightapSet a).
  apply (pr2 (issepp_dist _ _)).
  pattern a at 1 ; rewrite <- Ha.
  rewrite <- (Hb _ (isreflnatleh _)).
  rewrite (Hlx _ k).
  rewrite Ha, (Hb _ (isreflnatleh _)), issymm_dist.
  eapply istrans_NnMlt.
  apply H.
  pattern (dist (lx k) a) at 1 ;
    rewrite <- (lunax _ (dist (lx k) a)).
  now apply NnMplus_lt_r.
  easy.
  now apply natlthtoleh.
Qed.

Definition pointed_subdivision_Chasles (a b c : M)
           (s1 : pointed_subdivision a b) (s2 : pointed_subdivision b c) :
  (∀ k l : nat, (dist (ps_lx s1 k) (ps_lx s2 l) = dist (ps_lx s1 k) b + dist b (ps_lx s2 l))%addmonoid) ->
   pointed_subdivision a c.
Proof.
