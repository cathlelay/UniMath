  ...
  - destruct Yq as (q,(Yq,nYq)).
    ...
    + apply hinhpr ; right ; apply hinhpr.
      ...
      * intro Xq ; apply Xr.
        ...
        exact Xq.
        apply notle_ltNonnegativeRationals in Hdec.
...
Lemma Dcuts_gt_le_rel :
  ∀ x y : Dcuts_set, Dcuts_gt_rel x y -> Dcuts_le_rel x y.
Proof.
...
Lemma Dcuts_le_ngt_rel :
  ∀ x y : Dcuts_set, ¬ Dcuts_gt_rel x y <-> Dcuts_le_rel y x.
Proof.
  intros X Y.
  split.
  - intros Hngt y Yy.
    ...
    + intros (x,(Xx,Hx)).
      ...
      apply notlt_leNonnegativeRationals ; intro H ; apply Hngt.
      apply hinhpr.
      exists (x + (y' - y)) ; split.
      * exact Hx.
...
Lemma istrans_Dcuts_gt_le_rel :
  ∀ x y z : Dcuts_set, Dcuts_gt_rel x y -> Dcuts_le_rel y z -> Dcuts_gt_rel x z.
Proof.
  ...
  - exact Yr.
Qed.
Lemma istrans_Dcuts_le_gt_rel :
  ∀ x y z : Dcuts_set, Dcuts_le_rel x y -> Dcuts_gt_rel y z -> Dcuts_gt_rel x z.
Proof.
  intros x y z Hle.
...
Lemma iseo_Dcuts_le_gt_rel :
  isEffectiveOrder Dcuts_le_rel Dcuts_gt_rel.
Proof.
<<<<<<< HEAD
  split ; [ | split ; [ | repeat split]].
  - exact ispreorder_Dcuts_le_rel.
  - exact isstpo_Dcuts_gt_rel.
  - exact Dcuts_gt_le_rel.
  - now apply Dcuts_le_ngt_rel.
  - apply (pr2 (Dcuts_le_ngt_rel _ _)).
  - exact istrans_Dcuts_gt_le_rel.
  - exact istrans_Dcuts_le_gt_rel.
=======
  split.
  - split.
    + exact ispreorder_Dcuts_le_rel.
  ...
  - destruct Hx as (x,(Xx,nXx)).
    ...
    + assert (HY : ∀ y', coprod (Y y') (y' = 0%NRat) -> (y' < y + c / 2)%NRat).
      { intros y' [Yy' | Yy'].
        ...
        rewrite Yy'.
        now apply ispositive_plusNonnegativeRationals_r. }
      apply hinhpr ; right ; exists (x - (y + c / 2)) ; split.
      * apply hinhpr.
        exists x ; split.
        exact Xx.
        intros y' Yy'.
...
Proof.
  intros x y.
  rewrite !Dcuts_half_correct.
  now apply isrdistr_Dcuts_plus_mult.
Qed.
...
Proof.
  ...
  - apply hinhuniv ; intros [|] ; apply hinhuniv ; [intros [Xr | Yr] | intros ((rx,ry)) ; simpl ; intros (Hr,(Xr,Yr)) ].
    ...
    + apply is_Dcuts_bot with (1 := Yr).
      now apply plusNonnegativeRationals_le_r.
    + case (isdecrel_leNonnegativeRationals r rx) ; intro Hrx.
      now apply is_Dcuts_bot with (1 := Xr).
      apply notge_ltNonnegativeRationals in Hrx.
      apply is_Dcuts_bot with (1 := Yr).
  ...
  - destruct HuN as (q,(UNq,nUNq)).
    ...
    +  apply hinhpr ; right.
      ...
      * apply hinhpr.
        ...
        { apply (U_bot n) with (1 := Xr).
          pattern q at 2 ;
            rewrite <- (minusNonnegativeRationals_plus_r (c / 2) q).
          apply plusNonnegativeRationals_lecompat_l.
          pattern (c / 2) at 2 ;
        ...
        { apply (U_bot n) with (1 := Xr).
          ...
          rewrite <- Hr.
          pattern q at 2 ;
            rewrite <- (minusNonnegativeRationals_plus_r (c / 2) q), isassoc_plusNonnegativeRationals.
          apply plusNonnegativeRationals_lecompat_l.
          pattern (c / 2) at 2 ;
            rewrite (NQhalf_double (c / 2)).
          apply plusNonnegativeRationals_lecompat_l.
...
Proof.
  ...
  - eapply istrans_Dcuts_ge_gt_rel.
    ...
    + intros r Hr.
      ...
      * apply notlt_geNonnegativeRationals in Hrc.
        ...
        apply hinhpr ; right.
        apply hinhpr ; exists (r - c / 2, c / 2) ; repeat split.
  ...
  - apply istrans_Dcuts_gt_ge_rel with (Dcuts_plus (U N) (Dcuts_half eps)).
    ...
    + intros r.
      ...
      * apply notlt_geNonnegativeRationals in Hrc.
        apply hinhpr ; right.
        apply hinhpr ; exists (r - c / 2, c / 2) ; simpl ; repeat split.
        now rewrite minusNonnegativeRationals_plus_r.
